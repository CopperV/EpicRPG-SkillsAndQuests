...
public class PolnocnyBarbarzyncaItem extends SkillItem {

	public PolnocnyBarbarzyncaItem(Collection<IRequirement> requirements) {
		super("skill_polnocny_barbarzynca", "§2§lSkill: §e§oPolnocny barbarzynca", 10,
				requirements, Arrays.asList("§eZabijaj swoich wrogow przy uzyciu","§eswoich poteznych piesci"));
	}
	...
	public ItemStack getItem(Player p) {
		...
		if(rpg.getSkills().hasPolnocnyBarbarzynca())
			it = null;
		return it;
	}
	...
	public boolean clickAction(Player p, ItemStack info, EpicNPC npc) {
		...
		rpg.getSkills().setPolnocnyBarbarzynca(true);
		npc.openMenu(p);
		return true;
	}
	...
	public AEpicItem clone() {
		return new PolnocnyBarbarzyncaItem(new LinkedList<>(requirements));
	}
