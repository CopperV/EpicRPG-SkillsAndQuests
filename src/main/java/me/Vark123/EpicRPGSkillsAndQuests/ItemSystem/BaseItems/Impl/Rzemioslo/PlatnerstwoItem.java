...
public class PlatnerstwoItem extends LearnItem {

	public PlatnerstwoItem(Collection<IRequirement> requirements) {
		super("Platnerstwo", "§4§lRzemioslo: §cPlatnerstwo", 10, requirements);
	}
	...
	public ItemStack getItem(Player p) {
		...
		if(rpg.getRzemiosla().hasPlatnerstwo())
			it = null;
		return it;
	}
	...
	public boolean clickAction(Player p, ItemStack info, EpicNPC npc) {
		...
		p.sendMessage(npc.getName()+"§r: §aStworzenie pierwszej zbroi to istny banal. Problem zaczyna sie wtedy, gdy bedziesz miec do czynienia z metalem. Pozwol, ze Ci wytlumacze...");
		RpgPlayer rpg = PlayerManager.getInstance().getRpgPlayer(p);
		rpg.getInfo().removePN(price);
		rpg.getRzemiosla().setPlatnerstwo(true);
		npc.openMenu(p);
		return true;
	}
	...
	public AEpicItem clone() {
		return new PlatnerstwoItem(new LinkedList<>(requirements));
	}
